...
const clientProductRef = doc(totfd, "clientProductRef");
...
async function handleDeleteClient() {
	...
	const docSnapshot = await getDoc(clientProductRef);
	...
	if (docSnapshot.exists()) {
		...
		if (existingClientIndex !== -1) {
			...
			await updateDoc(clientProductRef, { clients: clientsData });
			nameInput.value = "";
			picInput.value = "";
			updateFormUI(false);
	...
	.addEventListener("submit", async function (event) {
		...
		if (existingClient) {
			...
			try {
				...
				if (pic) {
					var storageRef = ref(storage, "totfd/clientProductImages/" + pic.name);
				...
				if (docSnapshot.exists()) {
					...
					if (!existingClient) {
						...
						await updateDoc(clientProductRef, { clients });
					} else {
						console.log("Client already exists");
					}
				} else {
					await setDoc(clientProductRef, { clients: [{ name, picUrl }] });
				}
